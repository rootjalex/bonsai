  ...
  if (intersects_Ray_AABB(ray, (&_lv5))) {
    ...
    if ((distmin_Ray_AABB(ray, (&_lv4)) < std::get<0>((*_best0)))) {
      if ((*triangles).nodes[index].nprims == 0) {
        ...
        _recloop_func0((index + reinterpret<Arm_Interior>((*triangles).nodes[index].split0on_nprims).offset), triangles, ray, _best0);
      } else {
        for (uint32_t _idx0 = reinterpret<Arm_Leaf>((*triangles).nodes[index].split0on_nprims).poffset; _idx0 < (reinterpret<Arm_Leaf>((*triangles).nodes[index].split0on_nprims).poffset + (uint32_t)((*triangles).nodes[index].nprims)); _idx0 += 1) {
          const Triangle _lv3 = (*triangles).primitives[_idx0];
          if (intersectsp_ray_tri(ray, (&_lv3)).has_value()) {
            const Triangle _lv2 = (*triangles).primitives[_idx0];
            if ((distmin_Ray_Triangle(ray, (&_lv2)) < std::get<0>((*_best0)))) {
              const Triangle _lv0 = (*triangles).primitives[_idx0];
              const Triangle _lv1 = (*triangles).primitives[_idx0];
              (*_best0) = argmin<float, Triangle>(_best0, std::tuple<float, Triangle>{distmin_Ray_Triangle(ray, (&_lv0)), _lv1});
            }
          }
        }
      }
...
void rec_count_triangles(const BVH* node, Triangles* ST) {
  return std::visit(overloaded{
    [&](const Interior& node) {
