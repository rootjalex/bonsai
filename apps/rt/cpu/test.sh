...
echo "rays: ${RAY_COUNT}, runs: ${N}, hit ratio: ${HIT_RATIO}"
echo ${RAY_COUNT} >> ${DATA_PATH}/${DATA_FILE}.txt
for OBJECT in "${OBJECTS[@]}"; do
   echo "object: ${OBJECT}" 
  if [ ! -f "${RAY_PATH}/${OBJECT}.rays" ]; then
    echo "no rays found for ${OBJECT}; generating now..."
    ./${RAY_PATH}/${RAY_FILE}.out ${OBJECT} ${RAY_PATH} ${RAY_COUNT} ${HIT_RATIO}
    echo "...${RAY_COUNT} rays generated for ${OBJECT} with hit ratio: ${HIT_RATIO}"
  fi
  ...
  for LAYOUT in "${LAYOUTS[@]}"; do
    ...
    # 4. Run it.
    for ((i=0; i < N; i++)); do
      ./${PREFIX}/${APPLICATION}_${LAYOUT}.out ${OBJECT} ${RAY_COUNT} ${RAY_PATH}/${OBJECT}.rays >> ${DATA_PATH}/${DATA_FILE}.txt
    done
    # 5. Clean up
