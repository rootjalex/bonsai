...
__host__ void _recloop_func0(uint64_t I, Ray* ray, Triangles* triangles, cuda::std::tuple<float, Triangle>* _best0) {
  ...
  if (slice<0, 2>(I) == 1u) {
    ...
    for (uint64_t _idx0 = _t218; _idx0 < (_t218 + (uint64_t)(uint8_t)(slice<3, 6>(I) + 1u)); _idx0 += 1u) {
      ...
      if (intersectsp_ray_tri(ray, (&_t217)).has_value()) {
        ...
        if (_t215 < cuda::std::get<0>((*_best0))) {
          (*_best0) = argmin(_best0, cuda::std::tuple<float, Triangle>{_t215, _t217});
        }
      }
    }
  }
