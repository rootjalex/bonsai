...
__host__ float3 dequantize(uint32_t v, float3 bins) {
    ...
    return float3{fmul_rd((float)x_, bins.x), fmul_rd((float)y_, bins.y),
                  fmul_rd((float)z_, bins.z)};
}
...
_traverse_tree0(Ray *ray, Triangles *triangles) {
    ...
    do {
        ...
        AABB _t60 =
            AABB{__fadd_rd((*triangles).wlow, dequantize(_t48.q_min, _t50)),
                 __fsub_ru((*triangles).whigh, dequantize(_t48.q_max, _t50))};
            ...
            if (distmin_Ray_AABB(ray, (&_t60)) < cuda::std::get<0>(_best0)) {
                ...
                if (_t29 == 0u) {
                    ...
                    _queue0[(_queue_count0 + 1)] =
                        (index +
                         bonsai_reinterpret<Arm_Interior>(_t48.split0on_nprims)
                             .offset);
                    _queue_count0 += 2;
                } else {
                    uint32_t _t18 =
                        bonsai_reinterpret<Arm_Leaf>(_t48.split0on_nprims)
                            .poffset;
                    for (uint32_t _idx0 = _t18; _idx0 < (_t18 + (uint32_t)_t29);
                         _idx0 += 1u) {
...
__host__ cuda::std::optional<Triangle> *_traverse_array0(int64_t n, Ray *rays,
    ...
    Nodes *__nodes;
    cudaMallocAndCopyToDevice((void **)&__nodes, (*triangles).nodes,
                              (*triangles).node_count * sizeof(Nodes));
...
__host__ float3 build_bins_inverse(float3 low, float3 high) {
    float3 L1 = float3{fsub_ru(high.x, low.x), fsub_ru(high.y, low.y),
                       fsub_ru(high.z, low.z)};
    ...
    return float3{fdiv_rd((float)1023, L2.x), fdiv_rd((float)1023, L2.y),
                  fdiv_rd((float)1023, L2.z)};
}

__host__ float3 build_bins(float3 low, float3 high) {
    float3 bins_inverse = build_bins_inverse(low, high);
    return float3{frcp_rd(bins_inverse.x), frcp_rd(bins_inverse.y),
                  frcp_rd(bins_inverse.z)};
}

__host__ uint32_t quantize(float3 current, float3 world, float3 bin_inverse) {
    uint32_t x =
        (uint32_t)floorf(fmul_rd(fsub_rd(current.x, world.x), bin_inverse.x));
    uint32_t y =
        (uint32_t)floorf(fmul_rd(fsub_rd(current.y, world.y), bin_inverse.y));
    uint32_t z =
        (uint32_t)floorf(fmul_rd(fsub_rd(current.z, world.z), bin_inverse.z));
    return (uint32_t)(((x << 20u) | (y << 10u)) | z);
}

__host__ uint32_t rec_build_triangles(BVH *node_, Triangles *ST,
                                      size_t *nodes_index,
    ...
    if (std::holds_alternative<Interior>(*node_)) {
        ...
        (*ST).nodes[this_index].q_min =
            quantize(node.low, (*ST).wlow, (*ST).bins_inv);
        (*ST).nodes[this_index].q_max =
            quantize(node.high, (*ST).whigh, (*ST).bins_inv);
        (*ST).nodes[this_index].nprims = 0;
        uint32_t left_index =
            rec_build_triangles(node.left, ST, nodes_index, primitives_index);
        uint32_t right_index =
            rec_build_triangles(node.right, ST, nodes_index, primitives_index);
