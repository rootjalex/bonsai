...
void _recloop_func0(const uint32_t triangles1_index, const uint32_t triangles2_index, const _tree_layout0& triangles1, const _tree_layout4& triangles2, set<std::tuple<Triangle, Triangle>>& _dyn_alloc) {
  bonsai_rec_counter++;
  ...
  if (intersects_AABB_AABB(AABB{_t72.low, _t72.high}, AABB{_t76.low, _t76.high})) {
    ...
    if (_t70 == 0u) {
      if (_t76.nPrims == 0u) {
        ...
        _recloop_func0(triangles1_index + 1u, triangles2_index, triangles1, triangles2, _dyn_alloc);
        _recloop_func0(triangles1_index + _t72.offset, triangles2_index, triangles1, triangles2, _dyn_alloc);
      }
    } else {
      const uint32_t _t66 = _t76.nPrims;
      if (_t66 == 0u) {
        _recloop_func0(triangles1_index, triangles2_index + 1u, triangles1, triangles2, _dyn_alloc);
        _recloop_func0(triangles1_index, triangles2_index + _t76.offset, triangles1, triangles2, _dyn_alloc);
      } else {
        for (uint32_t _idx0 = 0u; _idx0 < _t70; _idx0 += 1u) {
          for (uint32_t _idx1 = 0u; _idx1 < _t66; _idx1 += 1u) {
            const Triangle _t39 = triangles1.prims[_t72.offset + _idx0];
            const Triangle _t45 = triangles2.prims[_t76.offset + _idx1];
...
set<std::tuple<Triangle, Triangle>> _traverse_tree0(const _tree_layout0& triangles1, const _tree_layout4& triangles2) {
