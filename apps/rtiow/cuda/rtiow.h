...
__device__ cuda::std::optional<MaterialSphere> _traverse_tree0(Ray* r, Spheres* spheres) {
  ...
  do {
    ...
    if (intersects_Ray_Sphere(r, (&_t54))) {
      if ((float)0.001 < distmax_Ray_Sphere(r, (&_t54))) {
        if (distmin_Ray_Sphere(r, (&_t54)) < cuda::std::get<0>(_best0)) {
          uint8_t _t32 = _t49.nprims;
          if (_t32 == 0u) {
            _queue0[_queue_count0] = (index + 1u);
            _queue0[(_queue_count0 + 1)] = (index + (uint32_t)bonsai_reinterpret<Arm_Interior>(_t49.split0on_nprims).offset);
            _queue_count0 += 2;
          } else {
            uint16_t _t21 = bonsai_reinterpret<Arm_Leaf>(_t49.split0on_nprims).poffset;
            for (uint16_t _idx0 = _t21; _idx0 < (_t21 + (uint16_t)_t32); _idx0 += 1u) {
              ...
              if (intersects_Ray_Sphere(r, (&_t16.s))) {
                ...
                if ((float)0.001 < _t14) {
                  if (_t14 < cuda::std::get<0>(_best0)) {
                    _best0 = argmin(_best0, cuda::std::tuple<float, MaterialSphere>{_t14, _t16});
                  }
                }
              }
            }
  ...
  return ((cuda::std::get<0>(_best0) != INFINITY) ? cuda::std::optional<MaterialSphere>{cuda::std::get<1>(_best0)} : cuda::std::nullopt);
}
...
__device__ float3 refract(float3 uv, float3 n, float etai_over_etat) {
  ...
  float3 _t2 = (make_float3(etai_over_etat) * (uv + (make_float3(cos_theta) * n)));
  float3 r_out_parallel = (make_float3(-sqrtf(fabsf(1 - sum((_t2 * _t2))))) * n);
...
__device__ Scatter_record scatter(Ray* ray, MaterialSphere* ms, curandState* _rng_state) {
  ...
  if (_t20 == 0u) {
    ...
    if (_t20 == 1u) {
      float3 ref = ((*ray).d - (make_float3(((float)2 * dot((*ray).d, _t1.normal))) * _t1.normal));
      ...
      float3 direction = (cannot_refract ? (_t14 - (make_float3(((float)2 * dot(_t14, _t15))) * _t15)) : refract(_t14, _t15, ri));
...
__global__ void _parfunc0(_ctx0 ctx0) {
  ...
  int3 _t0 = to_rgb(_traverse_array1(_i1, _i0, ctx0.c, ctx0.spheres, (&_rng_state)) / make_float3((float)(*ctx0.c).samples_per_pixel));
...
__host__ Sphere bounding_sphere(Sphere* a, Sphere* b) {
  ...
  float3 direction = ((0 < dist) ? (_t2 / make_float3(dist)) : make_float3(1, 0, 0));
...
__host__ Spheres build_spheres(BVH* CT) {
  ...
  Nodes* nodes;
  cudaMallocAndCopyToDevice((void**)&nodes, ST.nodes, (*ST).node_count * sizeof(Nodes));
  Spheres h_ST = ST;
  h_ST.primitives = primitives;
  h_ST.nodes = nodes;
  Spheres* d_ST;
