...
__host__ int3 *_traverse_array0(Camera *c, int32_t height, Spheres *spheres) {
    ...
    printf("Launching %d blocks of 512 threads\n",
           (height * (*c).width + 511) / 512);
    Camera *d_c;
    cudaMallocAndCopyToDevice((void **)&d_c, &(*c), sizeof(Camera));
    MaterialSphere *__primitives;
    cudaMallocAndCopyToDevice((void **)&__primitives, (*spheres).primitives,
                              (*spheres).primitive_count *
                                  sizeof(MaterialSphere));
    Nodes *__nodes;
    cudaMallocAndCopyToDevice((void **)&__nodes, (*spheres).nodes,
...
__host__ Spheres build_spheres(BVH *CT) {
    ...
    MaterialSphere *primitives = reinterpret_cast<MaterialSphere *>(
        malloc(sizeof(MaterialSphere) * ST.primitive_count));
