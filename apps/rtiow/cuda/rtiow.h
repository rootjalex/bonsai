...
struct Leaf {
  ...
  float radius;
  uint8_t nprims;
  MaterialSphere* data; // of size nprims
};
...
struct spheres {
  ...
  MaterialSphere* primitives; // of size primitive_count
  uint32_t node_count;
...
struct _ctx0 {
  ...
  Camera* c;
  spheres* spheres;
  int32_t* _alloc0; // of size ((height * c.width) * 3)
};
...
__device__ float3 random_in_unit_disk(curandState* _rng_state) {
  float r = sqrtf(curand_uniform(_rng_state));
  float theta = (((float)2 * (float)3.14159) * curand_uniform(_rng_state));
...
__device__ _option0 _traverse_tree0(Ray* r, spheres* spheres) {
  ...
  do {
    *_queue_count0 -= 1;
    ...
    Sphere _lv2 = Sphere{(*spheres).nodes[index].center, (*spheres).nodes[index].radius};
    if (intersects_Ray_Sphere(r, (&_lv2))) {
      Sphere _lv1 = Sphere{(*spheres).nodes[index].center, (*spheres).nodes[index].radius};
      if ((float)0.001 < distmax_Ray_Sphere(r, (&_lv1))) {
        Sphere _lv0 = Sphere{(*spheres).nodes[index].center, (*spheres).nodes[index].radius};
        if (distmin_Ray_Sphere(r, (&_lv0)) < _best0._field0) {
          if ((*spheres).nodes[index].nprims == 0u) {
            _queue0[_queue_count0] = (index + 1u);
            _queue0[(_queue_count0 + 1)] = (index + (uint32_t)bonsai_reinterpret<arm_Interior>((*spheres).nodes[index].split0on_nprims).offset);
            *_queue_count0 += 2;
          } else {
            for (uint16_t _idx0 = bonsai_reinterpret<arm_Leaf>((*spheres).nodes[index].split0on_nprims).poffset; _idx0 < (bonsai_reinterpret<arm_Leaf>((*spheres).nodes[index].split0on_nprims).poffset + (uint16_t)(*spheres).nodes[index].nprims); _idx0 += 1u) {
              if (intersects_Ray_Sphere(r, (&(*spheres).primitives[_idx0].s))) {
                if ((float)0.001 < distmin_Ray_Sphere(r, (&(*spheres).primitives[_idx0].s))) {
                  if (distmin_Ray_Sphere(r, (&(*spheres).primitives[_idx0].s)) < _best0._field0) {
                    *_best0 = argmin(_best0, __tuple_0{distmin_Ray_Sphere(r, (&(*spheres).primitives[_idx0].s)), (*spheres).primitives[_idx0]});
                  }
                }
              }
            }
...
__device__ scatter_record scatter(Ray* ray, MaterialSphere* ms, curandState* _rng_state) {
  ...
  if ((*ms).material == 0u) {
    float3 _scatter_dir = (hit.normal + random_unit_vector(_rng_state));
    ...
    if ((*ms).material == 1u) {
      ...
      float3 reflected = ((ref / make_float3(length(ref))) + (make_float3((*ms).fuzz) * random_unit_vector(_rng_state)));
...
__device__ float3 sample(Ray* r, int32_t depth, float3 mult, spheres* spheres, curandState* _rng_state) {
  ...
  do {
    ...
    if (isect.set) {
      scatter_record data = scatter((&S_r), (&isect.value), _rng_state);
      if (data.hit) {
        *S_r = data.ray;
        *S_depth = (S_depth - 1);
...
__global__ void _parfunc0(_ctx0 ctx0) {
  ...
  int32_t _i = tid;
  if ((ctx0.height * (*ctx0.c).width) <= _i) {
    return;
  }
  curandState _rng_state;
  curand_init(tid, 0, 0, &_rng_state);
  int32_t _i0 = (_i / (*ctx0.c).width);
  int32_t _i1 = (_i % (*ctx0.c).width);
  int3 __temp = to_rgb(_traverse_array1(_i1, _i0, ctx0.c, ctx0.spheres, (&_rng_state)) / make_float3((float)(*ctx0.c).samples_per_pixel));
  *ctx0._alloc0[(((_i0 * (*(*ctx0).c).width) + _i1) * 3)] = __temp.x;
  *ctx0._alloc0[((((_i0 * (*(*ctx0).c).width) + _i1) * 3) + 1)] = __temp.y;
  *ctx0._alloc0[((((_i0 * (*(*ctx0).c).width) + _i1) * 3) + 2)] = __temp.z;
  return;
}
...
__host__ int3* _traverse_array0(Camera* c, int32_t height, spheres* spheres) {
  ...
  nodes* nodes;
  cudaMallocAndCopyToDevice((void**)&nodes, (*spheres).nodes, (*spheres).node_count * sizeof(nodes));
...
__host__ uint32_t rec_build_spheres((const BVH&) node, spheres* ST, idx* nodes_index, idx* primitives_index) {
...
__host__ void rec_count_spheres((const BVH&) node, spheres* ST) {
  return std::visit(overloaded{
    [&](const Interior& node) {
...
spheres _unexported_build_spheres((const BVH&) CT) {
  spheres _ST;
  spheres* ST = &_ST;
  ...
  MaterialSphere* primitives;
  (void)cudaMalloc((void**)&primitives, ST.primitive_count * sizeof(MaterialSphere));
...
__host__ void build_spheres(spheres* _ret1, (const BVH&) CT) {
  spheres _ST;
  spheres* ST = &_ST;
  idx _primitives_index = 0;
  idx* primitives_index = &_primitives_index;
