...
vec3_float defocus_disk_sample(const vec3_float center,
                               const vec3_float defocus_disk_u,
                               const vec3_float defocus_disk_v) {
    const vec3_float p = random_in_unit_disk();
    return ((center + (vec3_float{p[0]} * defocus_disk_u)) +
            (vec3_float{p[1]} * defocus_disk_v));
}
Ray build_ray(const int32_t i, const int32_t j, const Camera *cam) {
    ...
    const vec3_float w = (((*cam).lookfrom - (*cam).lookat) /
                          vec3_float{norm(((*cam).lookfrom - (*cam).lookat))});
    const vec3_float u =
        (cross((*cam).vup, w) / vec3_float{norm(cross((*cam).vup, w))});
    ...
    const vec3_float viewport_upper_left =
        (((camera_center - (vec3_float{(*cam).focus_dist} * w)) -
          (viewport_u / vec3_float{2.00000000f})) -
         (viewport_v / vec3_float{2.00000000f}));
    const vec3_float pixel00_loc =
        (viewport_upper_left +
         (vec3_float{0.50000000f} * (pixel_delta_u + pixel_delta_v)));
    const float defocus_radius =
        ((*cam).focus_dist *
         tanf(((((*cam).defocus_angle / 2.00000000f) * 3.14159274f) /
               180.00000000f)));
    const vec3_float defocus_disk_u = (u * vec3_float{defocus_radius});
    const vec3_float defocus_disk_v = (v * vec3_float{defocus_radius});
    const vec3_float offset =
        vec3_float{(random_float<float>() - 0.50000000f),
                   (random_float<float>() - 0.50000000f), 0.0f};
    const vec3_float pixel_sample =
        ((pixel00_loc +
          (vec3_float{((float)(i) + offset[0])} * pixel_delta_u)) +
         (vec3_float{((float)(j) + offset[1])} * pixel_delta_v));
...
std::optional<FInterval> intersectsp_ray_sphere(const Ray *r, const Sphere *s) {
    ...
    const FInterval interval =
        FInterval{.low = min(root0, root1), .high = max(root0, root1)};
...
std::optional<MaterialSphere> _traverse_tree0(const Ray *r,
                                              const Spheres *spheres) {
    std::tuple<float, MaterialSphere> _best0 =
        std::tuple<float, MaterialSphere>{
            std::numeric_limits<float>::infinity(), MaterialSphere{}};
    ...
    do {
        ...
        const Sphere _lv2 = Sphere{.center = (*spheres).nodes[index].center,
                                   .radius = (*spheres).nodes[index].radius};
        if (intersects_Ray_Sphere(r, (&_lv2))) {
            const Sphere _lv1 =
                Sphere{.center = (*spheres).nodes[index].center,
                       .radius = (*spheres).nodes[index].radius};
            if ((0.00100000f < distmax_Ray_Sphere(r, (&_lv1)))) {
                const Sphere _lv0 =
                    Sphere{.center = (*spheres).nodes[index].center,
                           .radius = (*spheres).nodes[index].radius};
                if ((distmin_Ray_Sphere(r, (&_lv0)) < std::get<0>(_best0))) {
                    if ((*spheres).nodes[index].nprims == 0) {
                        _queue0[_queue_count0] = (index + 1);
                        _queue0[(_queue_count0 + 1)] =
                            (index + (uint32_t)(reinterpret<Arm_Interior>(
                                                    (*spheres)
                                                        .nodes[index]
                                                        .split0on_nprims)
                                                    .offset));
                        ...
                        for (uint16_t _idx0 =
                                 reinterpret<Arm_Leaf>(
                                     (*spheres).nodes[index].split0on_nprims)
                                     .poffset;
                             _idx0 <
                             (reinterpret<Arm_Leaf>(
                                  (*spheres).nodes[index].split0on_nprims)
                                  .poffset +
                              (uint16_t)((*spheres).nodes[index].nprims));
                             _idx0 += 1) {
                            if (intersects_Ray_Sphere(
                                    r, (&(*spheres).primitives[_idx0].s))) {
                                if ((0.00100000f <
                                     distmin_Ray_Sphere(
                                         r,
                                         (&(*spheres).primitives[_idx0].s)))) {
                                    if ((distmin_Ray_Sphere(
                                             r, (&(*spheres)
                                                      .primitives[_idx0]
                                                      .s)) <
                                         std::get<0>(_best0))) {
                                        _best0 = argmin<float, MaterialSphere>(
                                            _best0,
                                            std::tuple<float, MaterialSphere>{
                                                distmin_Ray_Sphere(
                                                    r, (&(*spheres)
    ...
    return ((std::get<0>(_best0) != std::numeric_limits<float>::infinity())
                ? std::optional<MaterialSphere>{std::get<1>(_best0)}
                : std::nullopt);
}
Hit_record get_hit_record(const Ray *r, const Sphere *s) {
    ...
    const vec3_float outward_normal =
        ((p - (*s).center) / vec3_float{(*s).radius});
    ...
    const Hit_record record =
        Hit_record{.p = p, .normal = normal, .t = t, .front_face = front_face};
    return record;
}
vec3_float random_unit_vector() {
    const float x1 =
        (-1.00000000f + ((1.0f - -1.00000000f) * random_float<float>()));
    const float x2 =
        (-1.00000000f + ((1.0f - -1.00000000f) * random_float<float>()));
...
Scatter_record scatter(const Ray *ray, const MaterialSphere *ms) {
    ...
    if ((*ms).material == 0) {
        ...
        if ((((abs(scatter_dir[0]) < 0.00000001f) &
              (abs(scatter_dir[1]) < 0.00000001f)) &
             (abs(scatter_dir[2]) < 0.00000001f))) {
            scatter_dir = hit.normal;
        }
        const Ray l_scattered =
            Ray{.o = hit.p,
                .d = scatter_dir,
                .tmax = std::numeric_limits<float>::infinity()};
        return Scatter_record{
            .attenuation = (*ms).albedo, .ray = l_scattered, .hit = true};
    } else if ((*ms).material == 1) {
        const vec3_float ref =
            ((*ray).d - (vec3_float{(2.00000000f * dot((*ray).d, hit.normal))} *
                         hit.normal));
        const vec3_float reflected =
            ((ref / vec3_float{norm(ref)}) +
             (vec3_float{(*ms).fuzz} * random_unit_vector()));
        const Ray m_scattered =
            Ray{.o = hit.p,
                .d = reflected,
                .tmax = std::numeric_limits<float>::infinity()};
        return Scatter_record{
            .attenuation = (*ms).albedo, .ray = m_scattered, .hit = true};
    } else {
        ...
        const bool cannot_refract =
            ((1.0f < (ri * sin_theta)) |
             (random_float<float>() < reflectance(cos_theta, ri)));
        const vec3_float direction =
            (cannot_refract
                 ? (unit_dir -
                    (vec3_float{(2.00000000f * dot(unit_dir, hit.normal))} *
                     hit.normal))
                 : refract(unit_dir, hit.normal, ri));
        const Ray d_scattered =
            Ray{.o = hit.p,
                .d = direction,
                .tmax = std::numeric_limits<float>::infinity()};
        return Scatter_record{.attenuation = vec3_float{1.0f, 1.0f, 1.0f},
                              .ray = d_scattered,
                              .hit = true};
    }
}
vec3_float sample(const Ray *r, const int32_t depth, const vec3_float mult,
                  const Spheres *spheres) {
    if ((depth <= 0)) {
    ...
    if (isect.has_value()) {
        ...
        if (data.hit) {
            return sample((&data.ray), (depth - 1), (mult * data.attenuation),
                          spheres);
        } else {
            return vec3_float{0.0f, 0.0f, 0.0f};
        }
    ...
    return (mult *
            ((vec3_float{(1.0f - a)} * vec3_float{1.0f, 1.0f, 1.0f}) +
             (vec3_float{a} * vec3_float{0.50000000f, 0.69999999f, 1.0f})));
}
vec3_float _traverse_array1(const int32_t i, const int32_t j, const Camera *c,
                            const Spheres *spheres) {
    vec3_float _alloc1 = vec3_float{0.0f};
    for (uint32_t _i0 = 0; _i0 < (*c).samples_per_pixel; _i0 += 1) {
        const Ray _lv0 = build_ray(i, j, c);
        _alloc1 += sample((&_lv0), (*c).max_depth, vec3_float{1.0f, 1.0f, 1.0f},
                          spheres);
    }
    return _alloc1;
}
...
vec3_int32_t to_rgb(const vec3_float v) {
    const vec3_float corrected =
        vec3_float{linear_to_gamma_f(v[0]), linear_to_gamma_f(v[1]),
                   linear_to_gamma_f(v[2])};
    return (vec3_int32_t)((
        vec3_float{256.00000000f} *
        min(max(corrected, vec3_float{0.0f}), vec3_float{0.99900001f})));
}
vec3_int32_t **_traverse_array0(const Camera *c, const int32_t height,
                                const Spheres *spheres) {
    int32_t *_alloc0;
    for (int32_t _i0 = 0; _i0 < height; _i0 += 1) {
        for (int32_t _i1 = 0; _i1 < (*c).width; _i1 += 1) {
            const vec3_int32_t __temp =
                to_rgb((_traverse_array1(_i1, _i0, c, spheres) /
                        vec3_float{(float)((*c).samples_per_pixel)}));
...
Sphere bounding_sphere(const Sphere *a, const Sphere *b) {
    ...
    const float new_radius =
        (0.50000000f * ((dist + (*a).radius) + (*b).radius));
    const vec3_float direction =
        ((0.0f < dist) ? (d / vec3_float{dist}) : vec3_float{1.0f, 0.0f, 0.0f});
    const vec3_float new_center =
        ((*a).center + (direction * vec3_float{(new_radius - (*a).radius)}));
    ...
    if ((vc <= 0.0f)) {
        if ((0.0f <= d1)) {
            if ((d3 <= 0.0f)) {
                ...
                return std::tuple<Point, Point>{
                    Point{.vec = (a + (vec3_float{v0} * ab))},
                    Point{.vec = vec3_float{(1.0f - v0), v0, 0.0f}}};
            }
        }
    }
    ...
    if ((vb <= 0.0f)) {
        if ((0.0f <= d2)) {
            if ((d6 <= 0.0f)) {
                ...
                return std::tuple<Point, Point>{
                    Point{.vec = (a + (vec3_float{w0} * ac))},
                    Point{.vec = vec3_float{(1.0f - w0), 0.0f, w0}}};
            }
        }
    }
    ...
    if ((va <= 0.0f)) {
        if ((0.0f <= (d4 - d3))) {
            if ((0.0f <= (d5 - d6))) {
                ...
                return std::tuple<Point, Point>{
                    Point{.vec = (b + (vec3_float{w1} * (c - b)))},
                    Point{.vec = vec3_float{0.0f, (1.0f - w1), w1}}};
            }
        }
    }
...
vec3_float cross_(const vec3_float v0, const vec3_float v1) {
    return vec3_float{__prod_diff_f32(v0[1], v1[2], v0[2], v1[1]),
                      __prod_diff_f32(v0[2], v1[0], v0[0], v1[2]),
                      __prod_diff_f32(v0[0], v1[1], v0[1], v1[0])};
}
...
std::optional<FInterval> intersectsp_ray_aabb(const Ray *r, const AABB *b) {
    ...
    tMax *= (1.0f + (2.00000000f * (((float)(3) * 0.00000006f) /
                                    (1.0f - ((float)(3) * 0.00000006f)))));
...
std::optional<TriangleIntersection> intersectsp_ray_tri(const Ray *ray,
                                                        const Triangle *tri) {
    if (reduce_add((cross_(((*tri).p2 - (*tri).p0), ((*tri).p1 - (*tri).p0)) *
                    cross_(((*tri).p2 - (*tri).p0),
                           ((*tri).p1 - (*tri).p0)))) == 0.0f) {
        return std::nullopt;
    }
    ...
    if (((det < 0.0f) &&
         ((0.0f <= tScaled) || (tScaled < ((*ray).tmax * det))))) {
        return std::nullopt;
    } else if (((0.0f < det) &&
                ((tScaled <= 0.0f) || (((*ray).tmax * det) < tScaled)))) {
        return std::nullopt;
    }
    ...
    const float deltaZ =
        ((((float)(3) * 0.00000006f) / (1.0f - ((float)(3) * 0.00000006f))) *
         maxZt);
    ...
    const float deltaX =
        ((((float)(5) * 0.00000006f) / (1.0f - ((float)(5) * 0.00000006f))) *
         (maxXt + maxZt));
    const float deltaY =
        ((((float)(5) * 0.00000006f) / (1.0f - ((float)(5) * 0.00000006f))) *
         (maxYt + maxZt));
    const float deltaE =
        (2.00000000f * (((((((float)(2) * 0.00000006f) /
                            (1.0f - ((float)(2) * 0.00000006f))) *
                           maxXt) *
                          maxYt) +
                         (deltaY * maxXt)) +
                        (deltaX * maxYt)));
    ...
    const float deltaT =
        ((3.00000000f * (((((((float)(3) * 0.00000006f) /
                             (1.0f - ((float)(3) * 0.00000006f))) *
                            maxE) *
                           maxZt) +
                          (deltaE * maxZt)) +
...
bool intersects_AABB_Triangle(const AABB *a, const Triangle *b) {
    const vec3_float center =
        (((*a).low + (*a).high) * vec3_float{0.50000000f});
    const vec3_float extents =
        (((*a).high - (*a).low) * vec3_float{0.50000000f});
...
vec3_float linear_to_gamma_v(const vec3_float l) {
    return vec3_float{linear_to_gamma_f(l[0]), linear_to_gamma_f(l[1]),
                      linear_to_gamma_f(l[2])};
}
bool near_zero(const vec3_float v) {
    return (((abs(v[0]) < 0.00000001f) & (abs(v[1]) < 0.00000001f)) &
            (abs(v[2]) < 0.00000001f));
}
vec3_float pixel(const int32_t i, const int32_t j, const Camera *c,
                 const Spheres *spheres) {
...
vec3_float random_vec3f() {
    return vec3_float{random_float<float>(), random_float<float>(),
                      random_float<float>()};
}
vec3_float random_vec3f_in(const float low, const float high) {
    return vec3_float{(low + ((high - low) * random_float<float>())),
                      (low + ((high - low) * random_float<float>())),
                      (low + ((high - low) * random_float<float>()))};
}
uint32_t rec_build_spheres(const BVH *node, Spheres *ST, uint32_t *nodes_index,
    ...
    node.match(
        [&](const Interior &node_Interior) {
            ...
            const uint32_t left_index = rec_build_spheres(
                (*node.left), ST, nodes_index, primitives_index);
            const uint32_t right_index = rec_build_spheres(
                (*node.right), ST, nodes_index, primitives_index);
            ST.nodes[this_index].split0on_nprims.offset =
                (right_index - this_index);
            return this_index;
        },
        [&](const Leaf &node_Leaf) {
            ...
            for (uint8_t __p = 0; __p < (*ST).nodes[this_index].nprims;
                 __p += 1) {
...
vec3_float sample_square() {
    return vec3_float{(random_float<float>() - 0.50000000f),
                      (random_float<float>() - 0.50000000f), 0.0f};
}
