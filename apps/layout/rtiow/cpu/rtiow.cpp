...
std::optional<MaterialSphere> _traverse_tree0(const Ray* r, const Spheres* spheres) {
  ...
  do {
    ...
    const Sphere _lv2 = Sphere{.center=(*spheres).nodes[index].center, .radius=(*spheres).nodes[index].radius};
    if (intersects_Ray_Sphere(r, (&_lv2))) {
      const Sphere _lv1 = Sphere{.center=(*spheres).nodes[index].center, .radius=(*spheres).nodes[index].radius};
      if ((0.00100000f < distmax_Ray_Sphere(r, (&_lv1)))) {
        const Sphere _lv0 = Sphere{.center=(*spheres).nodes[index].center, .radius=(*spheres).nodes[index].radius};
