                       ...
                       std::vector<bool>& isBoundaryVertex)
{
    ...
    // collect mesh indices; also count the number of times each triangle occurs in the tet mesh
    for (int i = 0; i < (int)Tet.rows(); i++) {
        meshIndices[i] = std::vector<size_t>{(size_t)Tet(i, 0), (size_t)Tet(i, 1),
                                             (size_t)Tet(i, 2), (size_t)Tet(i, 3)};

        triangleIndices.emplace_back(std::vector<size_t>{(size_t)Tet(i, 0), (size_t)Tet(i, 1), (size_t)Tet(i, 2)});
        triangleIndices.emplace_back(std::vector<size_t>{(size_t)Tet(i, 1), (size_t)Tet(i, 2), (size_t)Tet(i, 3)});
        triangleIndices.emplace_back(std::vector<size_t>{(size_t)Tet(i, 2), (size_t)Tet(i, 3), (size_t)Tet(i, 0)});
        triangleIndices.emplace_back(std::vector<size_t>{(size_t)Tet(i, 3), (size_t)Tet(i, 0), (size_t)Tet(i, 1)});
        std::sort(triangleIndices[4*i + 0].begin(), triangleIndices[4*i + 0].end());
        std::sort(triangleIndices[4*i + 1].begin(), triangleIndices[4*i + 1].end());
        std::sort(triangleIndices[4*i + 2].begin(), triangleIndices[4*i + 2].end());
        std::sort(triangleIndices[4*i + 3].begin(), triangleIndices[4*i + 3].end());
        triangleCountMap[triangleIndices[4*i + 0]] = triangleCountMap[triangleIndices[4*i + 0]] + 1;
        triangleCountMap[triangleIndices[4*i + 1]] = triangleCountMap[triangleIndices[4*i + 1]] + 1;
        triangleCountMap[triangleIndices[4*i + 2]] = triangleCountMap[triangleIndices[4*i + 2]] + 1;
        triangleCountMap[triangleIndices[4*i + 3]] = triangleCountMap[triangleIndices[4*i + 3]] + 1;
    }

    // extract the boundary
    for (auto it = triangleCountMap.begin(); it != triangleCountMap.end(); ++it) {
        if (it->second == 1) {
            ...
            boundaryIndices.emplace_back(Vector3i(index[0], index[1], index[2]));
        }
    }
...
zombie::EstimationQuantity getEstimationQuantity(zombie::SampleType type, bool estimateGradients)
{
...
template <size_t DIM>
void addMeshSampleAndEvaluationPoints(const std::vector<Vector<DIM>>& meshPositions,
                                      const std::vector<bool>& isBoundaryVertex,
                                      const zombie::GeometricQueries<DIM>& geometricQueries,
                                      int nWalksForSamplePts, bool estimateGradients,
                                      ...
                                      std::vector<zombie::SamplePoint<float, DIM>>& samplePoints)
{
    ...
    for (int i = 0; i < nV; i++) {
        ...
        zombie::SampleType type = isBoundaryVertex[i] ? zombie::SampleType::OnAbsorbingBoundary :
                                                        zombie::SampleType::InDomain;

        float distToAbsorbingBoundary = geometricQueries.computeDistToAbsorbingBoundary(pt, false);
        samplePoints.emplace_back(zombie::SamplePoint<float, DIM>(
            pt, Vector<DIM>::Zero(), type,
            getEstimationQuantity(type, estimateGradients),
...
template <size_t DIM>
void transformSlicePlane(const glm::mat4& objectTransform, std::vector<Vector<DIM>>& positions)
{
    glm::vec3 normal{objectTransform[0][0], objectTransform[0][1], objectTransform[0][2]};
    normal = glm::normalize(normal);

    for (int i = 0; i < (int)positions.size(); i++) {
        glm::vec4 coord(positions[i](0), positions[i](1), positions[i](2), 1.0f);
...
template <size_t DIM>
void addSlicePlaneSampleAndEvaluationPoints(const std::vector<Vector<DIM>>& slicePlanePositions,
                                            ...
                                            std::vector<zombie::SamplePoint<float, DIM>>& samplePoints)
{
    ...
    for (int i = 0; i < nF; i++) {
        ...
        center /= V;
        float distToAbsorbingBoundary = geometricQueries.computeDistToAbsorbingBoundary(center, false);
        samplePoints.emplace_back(zombie::SamplePoint<float, DIM>(
            center, Vector<DIM>::Zero(), zombie::SampleType::InDomain,
            getEstimationQuantity(zombie::SampleType::InDomain, estimateGradients),
            1.0f, distToAbsorbingBoundary, 0.0f));
    }
}

template <size_t DIM>
void plotSamplePoints(const std::vector<zombie::SamplePoint<float, DIM>>& samplePoints)
{
    ...
    if (DIM == 2) {
        polyscope::PointCloud *pointCloud = polyscope::registerPointCloud2D("Sample Points", points);
        pointCloud->addScalarQuantity("Boundary Distance", boundaryDistances);
        pointCloud->setEnabled(false);

    } else if (DIM == 3) {
        polyscope::PointCloud *pointCloud = polyscope::registerPointCloud("Sample Points", points);
        pointCloud->addScalarQuantity("Boundary Distance", boundaryDistances);
        pointCloud->setEnabled(false);
    }
}
...
template <size_t DIM>
void plotWalkOnSpheresEstimates(const std::vector<zombie::SamplePoint<float, DIM>>& samplePoints,
                                bool estimateOnSlicePlane, std::vector<float>& solution)
{
    ...
    std::vector<std::vector<float>> gradient(nPoints, std::vector<float>(DIM, 0.0f));
    std::vector<std::vector<float>> gradientVariance(nPoints, std::vector<float>(DIM, 0.0f));
    ...
    for (int i = 0; i < nPoints; i++) {
        ...
        const zombie::SampleStatistics<float, DIM>& statistics = samplePoints[i].statistics;
...
template <size_t DIM>
void maskWalkOnSpheresEstimates(float boundaryDistanceMask,
                                std::vector<zombie::SamplePoint<float, DIM>>& samplePoints)
{
                    ...
                    const std::vector<std::vector<float>>& gradient)
{
    ...
    if (DIM == 2) {
        ...
        polyscope::registerSurfaceMesh2D("FEM Mesh", meshPositionsRefined, meshIndicesRefined);
        ...
        polyscope::registerTetMesh("FEM Mesh", meshPositionsRefined, meshIndicesRefined);
...
template <size_t DIM>
void plotAbsoluteDifference(int nPoints,
                            const std::vector<float>& femSolution,
                            const std::vector<float>& mcSolution)
{
    ...
    if (DIM == 2) {
        polyscope::SurfaceMesh *mesh = polyscope::getSurfaceMesh("Mesh");
        mesh->addVertexScalarQuantity("Absolute Difference", absoluteDifference);
...
void clearResults(std::vector<zombie::SamplePoint<float, DIM>>& samplePoints)
{
    ...
    if (polyscope::state::slicePlanes.size() > 0 && polyscope::hasStructure("Surface Mesh", "Slice Plane")) {
        polyscope::SurfaceMesh *mesh = polyscope::getSurfaceMesh("Slice Plane");
        mesh->removeQuantity("Solution Estimate");
        mesh->removeQuantity("Gradient Estimate");
    }
                 ...
                 )
{
    ...
    if (ImGui::Checkbox("Ignore Source Contribution", &ignoreSourceContribution)) {
        clearResults<DIM>(samplePoints);
    }

    if (ImGui::Checkbox("Ignore Dirichlet Contribution", &ignoreDirichletContribution)) {
        clearResults<DIM>(samplePoints);
    }

    if (ImGui::SliderFloat("Absorption Coefficient", &absorptionCoeff, 0.0f, 100.0f, "%.1f", 2.5f)) {
    ...
    ImGui::SliderFloat("Epsilon Shell", &epsilonShell, 1e-4f, 1e-2f, "%.4f", 2.5f);
    ImGui::SliderFloat("Russian Roulette Threshold", &russianRouletteThreshold, 0.0f, 0.99f, "%.2f");
    ImGui::SliderFloat("Splitting Threshold", &splittingThreshold, 1.01f, 10.0f, "%.2f");
    ImGui::SliderInt("Max Walk Length", &maxWalkLength, 0, 10000);
    ImGui::SliderInt("Steps Before Applying Tikhonov", &stepsBeforeApplyingTikhonov, 0, maxWalkLength);
    if (ImGui::SliderFloat("Boundary Distance Mask", &boundaryDistanceMask, 0.0f, 0.1f, "%.4f", 2.5f)) {
    ...
    if (polyscope::state::slicePlanes.size() > 0 && polyscope::hasStructure("Surface Mesh", "Slice Plane")) {
        if (ImGui::SliderInt("Slice Plane Resolution", &slicePlaneResolution, 0, 5)) {
            std::vector<Vector<DIM>> slicePlanePositions;
            std::vector<std::vector<size_t>> slicePlaneIndices;
            glm::mat4 currentTransform = polyscope::state::slicePlanes[0]->getTransform();
            addSlicePlane<DIM>(geometricQueries.domainMin, geometricQueries.domainMax, currentTransform,
                               slicePlaneResolution, slicePlanePositions, slicePlaneIndices);
            addSlicePlaneSampleAndEvaluationPoints<DIM>(slicePlanePositions, slicePlaneIndices,
                                                        geometricQueries, nWalksForSamplePts,
                                                        estimateGradients, nWalks, samplePoints);
    ...
    if (!domainIsOpen) {
        if (ImGui::Checkbox("Estimate on Slice Plane", &estimateOnSlicePlane)) {
            if (estimateOnSlicePlane) {
                ...
                addMeshSampleAndEvaluationPoints<DIM>(meshPositions, isBoundaryVertex,
                                                      geometricQueries, nWalksForSamplePts,
                                                      estimateGradients, nWalks, samplePoints);
    ...
    ImGui::Checkbox("Disable Gradient Control Variates", &disableGradientControlVariates);
    ImGui::Checkbox("Disable Gradient Antithetic Variates", &disableGradientAntitheticVariates);
    ImGui::Checkbox("Use Cosine Sampling For Directional Derivatives", &useCosineSamplingForDirectionalDerivatives);
    ...
    if (!estimateOnSlicePlane || (estimateOnSlicePlane && solveDoubleSided)) {
        ...
        if (ImGui::SliderInt("Walks Per Sample Point", &nWalksForSamplePts, 1, 10000)) {
            std::fill(nWalks.begin(), nWalks.end(), nWalksForSamplePts);
        }
        if (ImGui::Checkbox("Estimate Gradients", &estimateGradients)) {
            for (int i = 0; i < (int)samplePoints.size(); i++) {
                samplePoints[i].estimationQuantity = getEstimationQuantity(samplePoints[i].type, estimateGradients);
            }
        ...
        if (ImGui::Button("Solve with Walk On Spheres")) {
            ...
            // std::cout << "  directionForDerivative: " << samplePoints[0].directionForDerivative.transpose() << "\n";
            // std::cout << "  type: " << static_cast<int>(samplePoints[0].type) << "\n";
            // std::cout << "  estimationQuantity: " << static_cast<int>(samplePoints[0].estimationQuantity) << "\n";
            // std::cout << "  pdf: " << samplePoints[0].pdf << "\n";
            // std::cout << "  distToAbsorbingBoundary: " << samplePoints[0].distToAbsorbingBoundary << "\n";
            // std::cout << "  distToReflectingBoundary: " << samplePoints[0].distToReflectingBoundary << "\n";
            // std::cout << "  firstSphereRadius: " << samplePoints[0].firstSphereRadius << "\n";
            // std::cout << "  robinCoeff: " << samplePoints[0].robinCoeff << "\n";
            // std::cout << "  solution: " << samplePoints[0].solution << "\n";
            // std::cout << "  normalDerivative: " << samplePoints[0].normalDerivative << "\n";
            ...
            if constexpr (DIM == 3) {
                for (uint64_t i = 0; i < samplePoints.size(); i++) {
                    bonsai_pts[i].pt = {samplePoints[i].pt(0), samplePoints[i].pt(1), samplePoints[i].pt(2)};
                    bonsai_pts[i].normal = {samplePoints[i].normal(0), samplePoints[i].normal(1), samplePoints[i].normal(2)};
                    bonsai_pts[i].pdf = samplePoints[i].pdf;
                    bonsai_pts[i].distToAbs = samplePoints[i].distToAbsorbingBoundary;
                    bonsai_pts[i].distToRefl = samplePoints[i].distToReflectingBoundary;
                    bonsai_pts[i].type_and_quantity = 0;
                    if (samplePoints[i].type == zombie::SampleType::InDomain) {
                        bonsai_pts[i].type_and_quantity |= 0;
                    } else if (samplePoints[i].type == zombie::SampleType::OnAbsorbingBoundary) {
                        bonsai_pts[i].type_and_quantity |= 1;
                    } else if (samplePoints[i].type == zombie::SampleType::OnReflectingBoundary) {
                        bonsai_pts[i].type_and_quantity |= 2;
                    }

                    if (samplePoints[i].estimationQuantity == zombie::EstimationQuantity::Solution) {
                        bonsai_pts[i].type_and_quantity |= 0;
                    } else if (samplePoints[i].estimationQuantity == zombie::EstimationQuantity::SolutionAndGradient) {
                        bonsai_pts[i].type_and_quantity |= 4;
                    } else if (samplePoints[i].estimationQuantity == zombie::EstimationQuantity::None) {
            ...
            zombie::WalkSettings walkSettings(epsilonShell, 0.0f, 0.0f,
                                              russianRouletteThreshold, splittingThreshold,
            ...
            if (false) {
                ...
                std::cout << "statistics.totalFirstSourceContribution = " << stats.totalFirstSourceContribution << "\n";
                std::cout << "statistics.totalDerivativeContribution = " << stats.totalDerivativeContribution << "\n";
                std::cout << "statistics.nSolutionEstimates = " << stats.nSolutionEstimates << "\n";
                std::cout << "statistics.nGradientEstimates = " << stats.nGradientEstimates << "\n";
                std::cout << "statistics.totalWalkLength = " << stats.totalWalkLength << "\n";
            ...
            if constexpr (DIM == 3) {
                ...
                bonsai_ws.stepsBeforeApplyingTikhonov = stepsBeforeApplyingTikhonov; // TODO: not used
                bonsai_ws.flags = (int)solveDoubleSided |
                ...
                if (false) {
                    ...
                    std::cout << "bonsai nSolutionEstimates = " << first.nSolEstimates << "\n";
                    std::cout << "bonsai totalWalkLength = " << first.totalWalkLength << "\n";
            ...
            for (int i = 0; i < (int)samplePoints.size(); i++) {
                ...
                // if (bonsai_solution && i < bonsai_fixed_count) {
                    ...
                    const bool equal =
                        ...
                        (samplePoints[i].statistics.solutionMean == bonsai_solution[i].solMean) &&
                        (samplePoints[i].statistics.solutionM2 == bonsai_solution[i].solMean2) &&
                        (samplePoints[i].statistics.totalFirstSourceContribution == bonsai_solution[i].totalFirstSourceContribution) &&
                        (samplePoints[i].statistics.nSolutionEstimates == bonsai_solution[i].nSolEstimates) &&
                        (samplePoints[i].statistics.totalWalkLength == bonsai_solution[i].totalWalkLength) &&
                        (samplePoints[i].statistics.totalSplits == bonsai_solution[i].totalSplits) &&
                        true;
        ...
        const std::string walksTakenPerSamplePtsStr = "Walks Taken Per Sample Point: " + std::to_string(nWalksTakenForSamplePts);
    ...
    if (!domainIsOpen) {
        ...
        // solve with FEM
        ImGui::SliderFloat("Refinement Multiplier", &femMeshRefinementMultiplier, 1.0f, 100.0f, "%.1f", 2.5f);
        if (ImGui::Button("Solve with FEM")) {
            ...
            femSolver.extractInterpolation(meshPositionsRefined, meshIndicesRefined, meshPositions,
                                           solutionRefined, gradientRefined, femSolution, femGradient);

            plotFemResults<DIM>(meshPositionsRefined, meshIndicesRefined, solutionRefined,
                                gradientRefined, femSolution, femGradient);
            if (!estimateOnSlicePlane) {
                plotAbsoluteDifference<DIM>(samplePoints.size(), walkOnSpheresSolution, femSolution);
            }
        }
                    ...
                    zombie::PDE<float, DIM>& pde)
{
    ...
    if (DIM == 2) {
        if (!domainIsOpen) polyscope::registerSurfaceMesh2D("Mesh", meshPositions, meshIndices);
        polyscope::registerCurveNetwork2D("Boundary", boundaryPositions, boundaryIndices);

    } else if (DIM == 3) {
        if (!domainIsOpen) polyscope::registerTetMesh("Mesh", meshPositions, meshIndices);
        polyscope::registerSurfaceMesh("Boundary", boundaryPositions, boundaryIndices);
    }
...
#ifdef USE_FEM
    std::vector<float> femSolution;
    polyscope::state::userCallback = std::bind(&guiCallback<DIM>, std::cref(meshPositions),
                                               std::cref(isBoundaryVertex), std::cref(geometricQueries),
                                               std::cref(walkOnSpheres), std::ref(femSolver),
                                               std::ref(pde), std::ref(nWalks), std::ref(samplePoints),
                                               std::ref(walkOnSpheresSolution), std::ref(femSolution));
#else
    polyscope::state::userCallback = std::bind(&guiCallback<DIM>, std::cref(meshPositions),
                                               std::cref(isBoundaryVertex), std::cref(geometricQueries),
                                               std::cref(walkOnSpheres), std::ref(pde), std::ref(nWalks),
...
void run()
{
    ...
    std::vector<Vectori<DIM>> boundaryIndices;
    if (domainIsOpen) {
        zombie::loadBoundaryMesh<DIM>(filename, boundaryPositions, boundaryIndices);
        zombie::normalize<DIM>(boundaryPositions);
        if (flipMeshOrientation) zombie::flipOrientation<DIM>(boundaryIndices);
        ...
        loadVolumeMesh<DIM>(filename, flipMeshOrientation, meshPositions, meshIndices,
                            boundaryPositions, boundaryIndices, isBoundaryVertex);
    }

    // compute bounding boxes
    std::pair<Vector<DIM>, Vector<DIM>> squareBoxExtents = zombie::computeBoundingBox<DIM>(meshPositions, true, 1.15f);
    std::pair<Vector<DIM>, Vector<DIM>> tightBoxExtents = zombie::computeBoundingBox<DIM>(meshPositions, false, 1.0f);
    auto tightBoxContains = [&tightBoxExtents](const Vector<DIM>& x) -> bool {
        ...
        return (x.array() >= bMin.array()).all() && (x.array() <= bMax.array()).all();
    };
    if (solveDoubleSided) {
        // add square box to geometry
        zombie::addBoundingBoxToBoundaryMesh<DIM>(squareBoxExtents.first, squareBoxExtents.second,
                                                  boundaryPositions, boundaryIndices);
    }

    // build boundary aggregate
    zombie::FcpwDirichletBoundaryHandler<DIM> boundaryHandler;
    boundaryHandler.buildAccelerationStructure(boundaryPositions, boundaryIndices);
    ...
    zombie::GeometricQueries<DIM> geometricQueries(!domainIsOpen, squareBoxExtents.first, squareBoxExtents.second);
    zombie::populateGeometricQueriesForDirichletBoundary<DIM>(boundaryHandler, geometricQueries);

    zombie::SdfGrid<DIM> sdfGrid(squareBoxExtents.first, squareBoxExtents.second);
    if (!solveDoubleSided && useSdfForBoundary) {
        // override distance queries to use an SDF grid. The user can also use Zombie to build an SDF hierarchy
        // for double-sided problems (ommited here for simplicity)
        ...
        zombie::populateGeometricQueriesForDirichletBoundary<zombie::SdfGrid<DIM>, DIM>(sdfGrid, geometricQueries);
    }
    ...
    pde.source = [freq](const Vector<DIM>& x) -> float {
        return ::ignoreSourceContribution ? 0.0f : 5.0f*std::sin(freq*x.y())*std::cos(freq*x.x());
    };
    pde.dirichlet = [freq, &tightBoxContains](const Vector<DIM>& x, bool returnBoundaryNormalAlignedValue) -> float {
        if (::solveDoubleSided && !tightBoxContains(x)) return 0.0f;
        return ::ignoreDirichletContribution ? 0.0f : std::sin(freq*x.x())*std::cos(freq*x.y());
    };
    ...
    // visualize the scene
    visualizeScene<DIM>(meshPositions, boundaryPositions, meshIndices, boundaryIndices,
                        isBoundaryVertex, geometricQueries, walkOnSpheres,
#ifdef USE_FEM
                        femSolver,
#endif
                        pde);
...
int main(int argc, const char *argv[])
{
    ...
    args::Flag flipMeshOrientation(group, "bool", "flip mesh orientation on load", {"flipMeshOrientation"});
    args::Flag domainIsOpen(group, "bool", "domain is open", {"domainIsOpen"});
    args::Flag solveDoubleSided(group, "bool", "solve double sided", {"solveDoubleSided"});
    args::Flag estimateGradients(group, "bool", "estimate gradients", {"estimateGradients"});
    args::Flag disableGradientControlVariates(group, "bool", "disable gradient control variates", {"disableGradientControlVariates"});
    args::Flag disableGradientAntitheticVariates(group, "bool", "disable gradient antithetic variates", {"disableGradientAntitheticVariates"});
    args::Flag useCosineSamplingForDirectionalDerivatives(group, "bool", "use cosine sampling for directional derivatives", {"useCosineSamplingForDirectionalDerivatives"});
    args::Flag ignoreDirichletContribution(group, "bool", "ignore Dirichlet values during random walk", {"ignoreDirichletContribution"});
    args::Flag ignoreSourceContribution(group, "bool", "ignore source values during random walk", {"ignoreSourceContribution"});
    args::Flag runSingleThreaded(group, "bool", "run single threaded", {"runSingleThreaded"});
    args::Flag printLogs(group, "bool", "print logs", {"printLogs"});
    args::Flag useSdfForBoundary(group, "bool", "use SDF for boundary", {"useSdfForBoundary"});
    args::ValueFlag<std::string> filename(parser, "string", "polygon soup filename", {"file"});
    args::ValueFlag<int> dim(parser, "int", "scene dimension", {"dim"});
    args::ValueFlag<int> nWalksForSamplePts(parser, "int", "walks per point for point estimator", {"nWalksForSamplePts"});
    args::ValueFlag<int> maxWalkLength(parser, "int", "max walk length for walk on stars", {"maxWalkLength"});
    args::ValueFlag<int> stepsBeforeApplyingTikhonov(parser, "int", "steps before applying Tikhonov regularization", {"stepsBeforeApplyingTikhonov"});
    args::ValueFlag<int> slicePlaneResolution(parser, "int", "slice plane resolution", {"slicePlaneResolution"});
    args::ValueFlag<int> sdfGridResolution(parser, "int", "SDF grid resolution", {"sdfGridResolution"});
    args::ValueFlag<float> absorptionCoeff(parser, "float", "absorption coefficient for screened Poisson eqution", {"absorptionCoeff"});
    args::ValueFlag<float> epsilonShell(parser, "float", "epsilon shell", {"epsilonShell"});
    args::ValueFlag<float> russianRouletteThreshold(parser, "float", "russian roulette threshold", {"russianRouletteThreshold"});
    args::ValueFlag<float> splittingThreshold(parser, "float", "splitting threshold", {"splittingThreshold"});
    args::ValueFlag<float> boundaryDistanceMask(parser, "float", "boundary distance for masking out estimated values", {"boundaryDistanceMask"});
#ifdef USE_FEM
    args::ValueFlag<float> femMeshRefinementMultiplier(parser, "float", "fem mesh refinement multiplier", {"femMeshRefinementMultiplier"});
#endif
    ...
    ::disableGradientControlVariates = args::get(disableGradientControlVariates);
    ::disableGradientAntitheticVariates = args::get(disableGradientAntitheticVariates);
    ::useCosineSamplingForDirectionalDerivatives = args::get(useCosineSamplingForDirectionalDerivatives);
    ...
    if (nWalksForSamplePts) ::nWalksForSamplePts = args::get(nWalksForSamplePts);
    if (maxWalkLength) ::maxWalkLength = args::get(maxWalkLength);
    if (stepsBeforeApplyingTikhonov) ::stepsBeforeApplyingTikhonov = args::get(stepsBeforeApplyingTikhonov);
    if (slicePlaneResolution) ::slicePlaneResolution = args::get(slicePlaneResolution);
    if (sdfGridResolution) ::sdfGridResolution = args::get(sdfGridResolution);
    if (absorptionCoeff) ::absorptionCoeff = args::get(absorptionCoeff);
    if (epsilonShell) ::epsilonShell = args::get(epsilonShell);
    if (russianRouletteThreshold) ::russianRouletteThreshold = args::get(russianRouletteThreshold);
    if (splittingThreshold) ::splittingThreshold = args::get(splittingThreshold);
    if (boundaryDistanceMask) ::boundaryDistanceMask = args::get(boundaryDistanceMask);
#ifdef USE_FEM
    if (femMeshRefinementMultiplier) ::femMeshRefinementMultiplier = args::get(femMeshRefinementMultiplier);
#endif
