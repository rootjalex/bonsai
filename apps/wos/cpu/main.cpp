...
void run_test(const std::string &obj1, int64_t num_queries) {
    ...
    // Bounding box of the mesh, used to place the query points.
    float3 bbox_min = triangles[0].p0;
    float3 bbox_max = triangles[0].p0;
    for (const Triangle &tri : triangles) {
        for (const float3 &v : {tri.p0, tri.p1, tri.p2}) {
            for (int i = 0; i < 3; i++) {
    ...
    for (int64_t i = 0; i < num_queries; i++) {
        ...
        for (int c = 0; c < 3; c++) {
            ...
            p.vec[c] = bbox_min[c] + (bbox_max[c] - bbox_min[c]) * r;
        }
    ...
    double checksum = 0.0;
    for (int64_t i = 0; i < num_queries; i++) {
        Triangle result = closest_point(&query_points[i], &tree);
        checksum += distmin(&query_points[i], &result);
    }
    t1 = clock::now();
    bonsai_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
    std::cout << "[bonsai] closest point query : " << bonsai_time << " ms"
              << std::endl;
    std::cout << "[bonsai] distance checksum   : " << checksum << std::endl;
}
