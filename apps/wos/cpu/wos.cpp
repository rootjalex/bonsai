...
void _recloop_func0(const uint32_t index, const Triangles* __restrict__ triangles, const Point* __restrict__ p, std::tuple<float, Triangle>* __restrict__ _best0) {
  if (index == 4294967295u) {
    return;
  }
  const Nodes& _t28 = (*triangles).nodes[index];
  const AABB _lv0 = AABB{.low=_t28.low, .high=_t28.high};
  if ((sqrtf(SqDistPointAABB(p, (&_lv0))) < std::get<0>((*_best0)))) {
    const uint16_t _t26 = _t28.nprims;
    if (_t26 == 0u) {
      _recloop_func0((index + 1u), triangles, p, _best0);
      _recloop_func0((index + reinterpret<Arm_Interior>(_t28.split0on_nprims).offset), triangles, p, _best0);
    } else {
      const uint32_t _t15 = reinterpret<Arm_Leaf>(_t28.split0on_nprims).poffset;
      for (uint32_t _idx0 = _t15; _idx0 < (_t15 + (uint32_t)(_t26)); ++_idx0) {
        const Triangle& _t9 = (*triangles).primitives[_idx0];
        const float _t10 = distmin_Point_Triangle(p, (&_t9));
        if ((_t10 < std::get<0>((*_best0)))) {
          (*_best0) = argmin<float, Triangle>(_best0, std::tuple<float, Triangle>{_t10, _t9});
        }
      }
    }
  }
...
Triangle _traverse_tree0(const Point* __restrict__ p, const Triangles* __restrict__ triangles) {
  ...
  _recloop_func0(0u, triangles, p, (&_best0));
...
Triangle closest(const Point* __restrict__ p, const Triangles* __restrict__ triangles) {
