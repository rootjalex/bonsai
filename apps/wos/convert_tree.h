...
_tree_layout0 convert_tree(const fcpwTriBVH *bvh) {
    ...
    auto build_triangle = [&](const uint64_t i) {
        ...
        const fcpw::Vector3& p0 = triangle->soup->positions[triangle->indices[0]];
        const fcpw::Vector3& p1 = triangle->soup->positions[triangle->indices[1]];
        const fcpw::Vector3& p2 = triangle->soup->positions[triangle->indices[2]];
        Triangle tri;
    ...
    for (size_t i = 0; i < tree.count; ++i) {
        ...
        tree.group0_index[i].low = {pMin(0), pMin(1), pMin(2)};
        // tree.group0_index[i].pad0 = 0;
        tree.group0_index[i].high = {pMax(0), pMax(1), pMax(2)};
        // TODO: set axis!
        // if (nPrimitives == 0) {
        ...
        //         return abs(max(left.box.pMax(dim) - right.box.pMin(dim), left.box.pMin(dim) - right.box.pMax(dim)));
        //     };
        ...
        if (nPrimitives > std::numeric_limits<uint16_t>::max()) {
            std::cerr << "(bonsai) Use larger index type for primitive count!\n";
            exit(-1);
        }
        ...
        *reinterpret_cast<uint16_t *>(&tree.group0_index[i].split0on_nPrims) = offset;
    }
    return tree;
}