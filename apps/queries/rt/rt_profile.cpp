...
std::optional<Triangle> _traverse_tree0(const Ray ray, const _tree_layout0& triangles) {
  std::tuple<float, Triangle> _best0 = std::make_tuple(std::numeric_limits<float>::infinity(), Triangle{});
      ...
      if (distmin_Ray_AABB(ray, _t50) < std::get<0>(_best0)) {
        ...
        if (_t35 == 0u) {
          ...
          for (uint32_t _idx0 = 0u; _idx0 < _t35; _idx0 += 1u) {
            const Triangle _t29 = triangles.prims[_t45.offset + _idx0];
  ...
  return ((std::get<0>(_best0) != std::numeric_limits<float>::infinity()) ? std::make_optional<Triangle>(std::get<1>(_best0)) : std::nullopt);
}
...
float3 cross_(const float3 v0, const float3 v1) {
  ...
  return vector<float, 3>({prod_diff(_t0, _t1, _t2, _t3), prod_diff(_t2, _t5, _t6, _t1), prod_diff(_t6, _t3, _t0, _t5)});
}
...
bool point_in_triangle(const float3 p, const Triangle t, const float3 q) {
  return ((orient3d(p, q, t.p0, t.p1) <= 0.00000000f) && (orient3d(p, q, t.p1, t.p2) <= 0.00000000f)) && (orient3d(p, q, t.p2, t.p0) <= 0.00000000f);
}
float sqlen(const float3 v) {
