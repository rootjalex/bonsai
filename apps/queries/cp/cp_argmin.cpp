...
Triangle _traverse_tree0(const Point& p, const _tree_layout0& tris) {
  ...
  do {
    ...
    if (distmin_Point_AABB(p, _t91) < std::get<0>(_best0)) {
      ...
      if (_t84 == 0u) {
        _best0 = argmin<float, Triangle>(_best0, std::tuple<float, Triangle>{distmax_Point_AABB(p, _t91), Triangle{}});
        const uint32_t _t11 = tris_index + _t86.offset;
        const uint32_t _t18 = tris_index + 1u;
        const bool _sort_cmp0 = distmin_Point_AABB(p, AABB{_t85[_t11].low, _t85[_t11].high}) < distmin_Point_AABB(p, AABB{_t85[_t18].low, _t85[_t18].high});
