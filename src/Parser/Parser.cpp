    ...
    void parse_rewrites(ir::Schedule &schedule, std::string func) {
        do {
            ...
            if (rewrite == "bound") {
                ...
                expect(Token::Type::COMMA);
                ir::schedule::Location ij = parse_location();
                schedule.rewrites.emplace_back(
                    func, ir::schedule::Collapse{std::move(i), std::move(j),
                                                 std::move(ij)});
            } else if (rewrite == "defer") {
                ir::schedule::Location i = parse_location();
                expect(Token::Type::COMMA);
                ir::schedule::Location queue = parse_location();
                schedule.rewrites.emplace_back(
                    func, ir::schedule::Defer{std::move(i), std::move(queue)});
                // TODO: vectorize(), cpu_thread(), gpu_thread(), gpu_block()
                // TOODO: prefetch(), reorder(), sort(), split(), synchronize(),
                // unroll()
            } else {
                report_error()
