...
struct Parser {
  ...
  private:
    ...
    ir::schedule::Location parse_location() {
        ir::schedule::Location loc;
        do {
            std::string name = get_id();
    ...
    void parse_rewrites(ir::Schedule &schedule, std::string func) {
        do {
            ...
            if (rewrite == "bound") {
                ir::Expr value = parse_expr();
                schedule.rewrites.emplace_back(
                    func, ir::schedule::Bound{std::move(value)});
            } else if (rewrite == "reorder") {
                ir::schedule::Location i = parse_location();
                expect(Token::Type::COMMA);
                ir::schedule::Location j = parse_location();
                schedule.rewrites.emplace_back(
                    func, ir::schedule::Reorder{std::move(i), std::move(j)});
            } else if (rewrite == "split") {
                ir::schedule::Location i = parse_location();
                expect(Token::Type::COMMA);
                ir::schedule::Location io = parse_location();
                expect(Token::Type::COMMA);
                ir::schedule::Location ii = parse_location();
                expect(Token::Type::COMMA);
                ir::Expr factor = parse_base_expr();
                if (!factor.defined() || !factor.is<ir::IntImm>()) {
                    report_error() << "split() expects an integer value for "
                                      "parameter `factor`, instead parsed: "
                                   << factor;
                }
                expect(Token::Type::COMMA);
                ir::Expr exact = parse_base_expr();
                if (!exact.defined() || !exact.is<ir::BoolImm>()) {
                    report_error() << "split() expects a boolean value for "
                                      "parameter `exact`, instead parsed: "
                                   << exact;
                }
                schedule.rewrites.emplace_back(
                    func, ir::schedule::Split{std::move(i), std::move(io),
                                              std::move(ii),
                                              factor.as<ir::IntImm>()->value,
                                              is_const_one(exact)});
            } else if (rewrite == "vectorize") {
                ir::schedule::Location i = parse_location();
                schedule.rewrites.emplace_back(
                    func,
                    ir::schedule::Parallelize{
                        std::move(i), ir::schedule::Parallelize::CPUVector});

                // TODO: cpu_thread(), gpu_thread(), gpu_block(), prefetch(),
                // TODO: sort(), synchronize(), unroll(), collapse(), defer()
            } else {
                report_error()
