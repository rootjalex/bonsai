...
struct UnswitchImpl : public Mutator {
    ...
    Stmt visit(const Sequence *node) override {
        ...
        for (size_t i = 0; i < node->stmts.size(); ++i) {
            ...
            // Try to merge with previous statement if both are ifs with same cond.
            if (!new_stmts.empty()) {
                const auto *prev_if = new_stmts.back().as<IfElse>();
                const auto *curr_if = s.as<IfElse>();

                // Equal if statements.
                // Can only merge if iff conditions are equal, and the body of the first does not mutate
                // any variable read in the condition of the second.
                if (prev_if && curr_if && equals(prev_if->cond, curr_if->cond) && !reads(curr_if->cond, mutated_variables(new_stmts.back()))) {

                    // Useful helper for merging flat sequences.
                    auto insert_sequence = [](std::vector<Stmt> &acc, Stmt stmt) {
                        if (!stmt.defined()) {
                            return;
                        }
                        const Sequence *seq = stmt.as<Sequence>();
                        if (seq) {
                            acc.insert(acc.end(), seq->stmts.begin(), seq->stmts.end());
                        } else {
                    ...
                    internal_assert(!merged_then_body.empty() && merged_then_body.size() > 1);

                    Stmt merged_then = Sequence::make(std::move(merged_then_body));
                    // Try to recursively mutate, this sequence may be merge-able as well.
                    ...
                    if (!merged_else_body.empty()) {
                        if (merged_else_body.size() == 1) {
                            ...
                            merged_else = Sequence::make(std::move(merged_else_body));
                            // Try to recursively mutate, this sequence may be merge-able as well.
                            merged_else = mutate(merged_else);
                        }
                    }
                    Stmt new_if = IfElse::make(prev_if->cond, std::move(merged_then), std::move(merged_else));
    ...
    // forall var in (...) { if (a) { body } } => if (a) { forall var in (...) { body }}
    // if and only if a does not use var!
    Stmt visit(const ForAll *node) override {
        ...
        if (const IfElse *if_else = node->body.as<IfElse>()) {
            ...
            if (!reads(if_else->cond, varying)) {
                ...
                ir::Stmt body0 = ForAll::make(node->index, node->header, node->slice, if_else->then_body);
                ir::Stmt body1 = if_else->else_body.defined() ? ForAll::make(node->index, node->header, node->slice, if_else->else_body) : if_else->else_body;
                return IfElse::make(if_else->cond, std::move(body0), std::move(body1));
            }
        }
...
Stmt unswitch_stmt(Stmt stmt) {
    return UnswitchImpl().mutate(std::move(stmt));
}
} // namespace
