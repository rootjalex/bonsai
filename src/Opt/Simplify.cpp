...
struct Simplifier : ir::Mutator {
    ir::Expr visit(const ir::BinOp *node) override {
        ...
        ir::Type type = a.type();
        if (!type.is_scalar()) {
            // TODO(cgyurgyik): extend constant folding to
            // vectors and constant-sized arrays.
            return node;
        }
        ...
        case ir::BinOp::OpType::Add: {
            if (c_a.has_value() && c_b.has_value()) {
                if (a.type().is_int()) {
                    return ir::IntImm::make(
                        std::move(type),
                        apply<int64_t>(std::plus<>{}, *c_a, *c_b));
                }
                if (a.type().is_uint()) {
                    return ir::UIntImm::make(
                        std::move(type),
                        apply<uint64_t>(std::plus<>{}, *c_a, *c_b));
                }
                if (a.type().is_float()) {
                    return ir::FloatImm::make(
                        std::move(type),
                        apply<double>(std::plus<>{}, *c_a, *c_b));
                }
            }
        ...
        case ir::BinOp::OpType::Mul: {
            if (c_a.has_value() && c_b.has_value()) {
                if (a.type().is_int()) {
                    return ir::IntImm::make(
                        std::move(type),
                        apply<int64_t>(std::multiplies<>{}, *c_a, *c_b));
                }
                if (a.type().is_uint()) {
                    return ir::UIntImm::make(
                        std::move(type),
                        apply<uint64_t>(std::multiplies<>{}, *c_a, *c_b));
                }
                if (a.type().is_float()) {
                    return ir::FloatImm::make(
                        std::move(type),
                        apply<double>(std::multiplies<>{}, *c_a, *c_b));
                }
            }
            if ((c_a.has_value() && *c_a == 0) ||
                (c_b.has_value() && *c_b == 0)) {
