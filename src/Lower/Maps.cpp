...
struct BuildMapArgs {
    std::string result;
    std::set<std::string> packet_array_list; // names of arrays to `packet`
    std::set<size_t> packet_loop_list; // idx of loops to `packet`
    // TODO: can also have string idx identifiers for loops after supporting split()
};

// TODO: make sure this works for i32, i64, u64, etc...
Expr stride_index(Expr base, Expr stride, Expr offset) {
    // return cast(index_t, base) * cast(index_t, stride) + cast(index_t, offset);
    return base * stride + offset;
}

// TODO: dedup with Opt/Fusion.cpp
struct Map {
    Expr lambda;
    Expr array;
};
...
Stmt build_traversal_helper(const Expr &func, const Expr &array, const size_t depth, const Expr &out_idx, const BuildMapArgs &args, FuncMap &funcs) {
    ...
    if (const Lambda *l = func.as<Lambda>()) {
        internal_assert(l->args.size() == 1)
            << "[unimplemented] map fusion for tuple splitting:" << func << " called with " << load;
        Expr expr = replace(l->args[0].name, load, l->value);
        // Try to fuse a nested map.
        if (auto map = as_map(expr)) {
            auto [next_func, next_array] = *map;
            loop_body = build_traversal_helper(next_func, next_array, depth + 1, nested_idx, args, funcs);
        } else {
            loop_body = Store::make(args.result, nested_idx, expr, /*predicate=*/Expr());
        }
    } else {
    ...
    return ForAll::make(loop_idx, std::move(header), ForAll::Slice{std::move(begin), std::move(end), std::move(stride)}, std::move(loop_body));
}
...
Stmt build_traversal(const SetOp *map_expr, FuncMap &funcs) {
    ...
    Stmt body = build_traversal_helper(map_expr->a, map_expr->b, /*depth=*/0, zero, args, funcs);
    ...
    return Sequence::make({std::move(alloc), std::move(body), std::move(return_var)});
}
...
struct LowerMapsImpl : public Mutator {
    ...
    Expr build_func(const SetOp *node) {
        ...
        internal_assert(inserted) << "Function: " << func << " already exists in program funcs\n";
        ...
        return Call::make(Var::make(std::move(call_type), func),
                              call_args);
    }
};
