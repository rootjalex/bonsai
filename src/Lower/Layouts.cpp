...
std::string unique_pad_name() {
    return "?pad" + std::to_string(pad_counter++); 
}
...
std::vector<ir::Type> layout_to_structs(std::string base, const ir::Layout &layout) {
    ...
    if (const ir::Chain *chain = layout.as<ir::Chain>()) {
        ...
        for (const auto &l : chain->layouts) {
            switch (l.node_type()) {
                ...
                case ir::IRLayoutEnum::Group: {
                    ...
                    std::string new_base = node->name.empty() ? "" : base + "_" + node->name;
                    std::vector<ir::Type> rec = layout_to_structs(new_base, node->inner);
                    internal_assert(!rec.empty());
                    ir::Type base_t = rec.back();
                    ir::Type group_t = ir::Array_t::make(std::move(base_t), node->size);
                    // Add to rets.
                    rets.insert(rets.end(), std::make_move_iterator(rec.begin()), std::make_move_iterator(rec.end()));
                    std::string field_name = new_base.empty() ? unique_group_name() : new_base;
                ...
                case ir::IRLayoutEnum::Split: {
                    ...
                    // Store as vector of bytes, load and reinterpret to proper type.
                    const uint64_t bits = l.bits();
                    internal_assert(bits % 8 == 0) << "Split is not byte-aligned: " << l;
                    static const ir::Type u8 = ir::UInt_t::make(8);
                    ir::Type byte_vec = ir::Vector_t::make(u8, bits / 8);
                    fields.emplace_back(unique_split_name(node->field), std::move(byte_vec));
                    break;
                }
                default: {
        ...
        rets.push_back(ir::Struct_t::make(unique_struct_name(std::move(base)), std::move(fields)));
        return rets;
    }
