    ...
    struct DeferImpl : ir::Mutator {
        ...
        Stmt visit(const RecLoop *node) override {
            ...
            for (int i = 0, e = node->args.size(); i < e; ++i) {
                const ir::Argument &v = node->args[i];
            ...
            if (!queue_size.defined()) {
                // Case 1: this is a dynamic array.
                static const Expr capacity = ir::IdxImm::make(128);
                Type array_t =
                    ir::DynArray_t::make(Tuple_t::make(types), capacity);
            }
            // Case 2: this is a static array.
            ...
            stmts.push_back(Allocate::make(
                /*loc=*/loc,
                /*value=*/Build::make(queue_type, std::vector<ir::Expr>{}),
...
Program LowerPostLayoutDefers::run(Program program,
