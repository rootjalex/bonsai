...
ir::Stmt build_arg_extremum(Extremum dir, ir::Expr metric, ir::Expr inner,
    ...
    struct RewriteArgExtremum : public Rewriter {
        ...
        // already guarded and this rewrite must tighten the accumulator.
        ...
        ir::Stmt visit(const ir::Yield *node) override {
            ...
            // Filter fusion has already established that any value reaching
            // here beats the running best, so this can store rather than
            // compare.
        ...
        // from tr => upd a arg(a, (max(M, tr), null)); from tr
        ir::Stmt visit(const ir::YieldFrom *node) override {
            if (!update_from_yfs) {
                return node;
            }
            ...
            // No element is attached to the bound; it only tightens the
            // metric so that worse subtrees are skipped.
            ir::Expr empty =
                ir::Build::make(tuple_t.as<ir::Tuple_t>()->etypes[1]);
            std::vector<ir::Expr> values = {std::move(bound), std::move(empty)};
            ir::Stmt do_update = ir::Accumulate::make(
                loc,
                dir == Extremum::Min ? ir::Accumulate::Argmin
                                     : ir::Accumulate::Argmax,
                ir::Build::make(tuple_t, std::move(values)));
...
ir::Stmt build_extremum(Extremum dir, ir::Expr metric, ir::Expr inner,
    ...
    struct RewriteExtremum : public Rewriter {
        ...
        // Order two children by how promising they are, and skip either when
        // the other is strictly better. Only written for minima; a maximum
        // falls back to the whole-node bound above.
        ir::Stmt visit_children_in_order(const ir::YieldFrom *node,
                                         const ir::Lambda *lambda) {
            const auto as = break_tuple(node->value);
            std::vector<ir::Expr> uppers, lowers;
            for (const auto &a : as) {
                VolumeMap cvols = make_volume_map(lambda->args, a);
                Interval cb = predicate_analysis(lambda->value, cvols,
                                                 intervals);
                internal_assert(cb.min.defined() && cb.max.defined())
                    << cb.min << " and " << cb.max;
                uppers.push_back(std::move(cb.max));
                lowers.push_back(std::move(cb.min));
            }

            ir::Expr upper_bound = uppers[0];
            for (size_t i = 1; i < as.size(); i++) {
                upper_bound = max(std::move(upper_bound), uppers[i]);
            }

            ir::Stmt body = node;
            if (as.size() >= 2) {
                internal_assert(as.size() == 2) << ir::Stmt(node); // TODO
                // If one child is strictly better than the other, only visit
                // it; otherwise visit the closer one first.
                ir::Expr a_winner = uppers[0] < lowers[1];
                ir::Expr b_winner = uppers[1] < lowers[0];
                ir::Expr a_closer = (lowers[0] < lowers[1]) |
                                    (lowers[0] == lowers[1]) &
                                        (uppers[0] < uppers[1]);
                body = ir::IfElse::make(
            ...
            ir::Stmt do_update = ir::Accumulate::make(loc, accumulate_op(),
        ...
        ir::Stmt visit(const ir::YieldFrom *node) override {
            ...
            // When a node annotates its children's volumes rather than its
            // own, each child can be bounded separately, which both tightens
            // the accumulator and lets the more promising child go first.
            if (!child_volumes.empty() && dir == Extremum::Min) {
                return visit_children_in_order(node, lambda);
            }
...
struct LowerBVH : public ir::Mutator {
    ...
    ir::Expr visit(const ir::SetOp *op) override {
        if (op->op == ir::SetOp::map) {
            // TODO: support map fusion. A map over an array is handled by
            // LowerMaps; one over a tree is reached through the reduction
            // that consumes it.
...
ir::Stmt build_product(ir::Stmt a_body, ir::Stmt b_body) {
    struct RewriteProduct : public Rewriter {
        ...
        RewriteProduct(ir::Stmt b_body) : b_body(std::move(b_body)) {}
    ...
    return RewriteProduct(std::move(b_body)).mutate(a_body);
}
