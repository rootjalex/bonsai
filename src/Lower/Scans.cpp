...
std::string scan_func_name(const std::vector<ir::Argument> &args) {
...
build_scan_func(const std::vector<ir::Argument> &args) {
    ...
    struct ScansToCalls : public Mutator {
        ...
        Stmt visit(const Iterate *node) override {
            return AppendStmt::make(write_loc, node->value);
        }
...
struct LowerScansImpl : public Mutator {
    ...
    Expr get_or_build_callable() {
        std::vector<ir::Argument> scan_args;
    ...
    Stmt visit(const RecLoop *node) override {
        ...
        for (const auto &fv : free_vars) {
            if (auto [_, inserted] = arg_names.insert(fv.name); inserted) {
                args.push_back(ir::Argument(fv.name, fv.type));
            }
        }
    ...
    Stmt visit(const Scan *node) override {
        ...
        internal_assert(args.front().type.is<BVH_t>()) << args;
        ...
        for (const auto &id : ids) {
            ...
            const ir::Argument &back = args.back();
            call_args.push_back(ir::Var::make(back.type, back.name));
