...
class BonsaiToCpp : ir::Printer {
  ...
  private:
        ...
        if (node->value.is<Extract>()) {
            ...
            if (const auto *struct_t = node->value.type().as<Struct_t>();
                struct_t && (!struct_t->name.starts_with("_") ||
                             struct_t->name.starts_with("_tree"))) {
    ...
    // test of the tag per arm, so they are not emitted: the base printer
    // would print them as bonsai rather than as C++, which is the silent kind
    // of wrong.
