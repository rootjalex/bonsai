...
void CodeGen_LLVM::visit(const Launch *node) {
    ...
    llvm::Value *global_dispatch_queue;
    {
        ...
        dispatch_get_global_queue_functionParams.push_back(i64_t);
        llvm::FunctionType *dispatch_get_global_queue_functionType =
        ...
        if (!dispatch_get_global_queue_function) {
            dispatch_get_global_queue_function = llvm::Function::Create(
                dispatch_get_global_queue_functionType,
                llvm::GlobalValue::ExternalLinkage, "dispatch_get_global_queue",
                module.get());
        }

        llvm::Constant *zeroValue = llvm::ConstantInt::get(
            dispatch_get_global_queue_functionType->getParamType(0), 0);
