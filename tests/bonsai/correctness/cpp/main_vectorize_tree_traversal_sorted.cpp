#include "vectorize_tree_traversal_sorted.h"
...
// Used in vectorize_tree_traversal_sorted.bonsai.
//
// main_vectorize_tree_traversal.cpp's scene and tree, and its eight rays --
// except that each ray leans very slightly along z, alternately up and down
// from lane to lane. The shapes lie along z, so z is the axis every node is
// split on, and the lean is what the sorted traversal's key looks at: lanes
// of the same gang then descend into the two children in opposite orders,
// which is the case the sort has to get right lane by lane. The lean is far
// too small to change what any ray hits.
namespace {
