#include "vectorize_tree_traversal_packet.h"
...
// Used in vectorize_tree_traversal_packet.bonsai.
//
// main_vectorize_tree_traversal_sorted.cpp's scene, tree and rays: eight rays
// along x at different heights, each leaning very slightly along z,
// alternately up and down from lane to lane. Under the packet schedule the
// gang shares one node and decides the order of its children by a vote, so
// half the lanes descend in the order their own key would not have chosen.
// That is allowed to cost them a subtree visited early, and it must not
// change what any of them hits: every lane's answer has to match the same
// ray traced alone by the scalar `hit_z_one`.
namespace {
...
int main() {
    ...
    for (size_t i = 0; i < rays.size(); i++) {
        const float lean = (i % 2 == 0) ? 1e-3f : -1e-3f;
        rays[i] = Ray{float3{-10.0f, 0.0f, heights[i]}, float3{1.0f, 0.0f, lean}};
    }
