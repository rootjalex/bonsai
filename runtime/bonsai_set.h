...
public:
    ...
    void push_back(const range<T, U> &range) {
        // std::unique_lock lock(mutex);
        data.insert(data.end(), range.data + range.offset, range.data + range.offset + range.count);
    }
...
U reduce(std::function<U(const U&, const T&)> reducer, const set<T>& input, U initial) {
    input.for_each([&](const T& item) {
...
template<typename T, typename U>
set<std::tuple<T, U>> nested_join(std::function<bool(const T&, const U&)> predicate, const set<T> &input0, const set<U> &input1) {
    std::vector<std::tuple<T, U>> result;
    input0.for_each([&](const T& item0) {
        input1.for_each([&](const U& item1) {
