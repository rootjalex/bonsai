...
if (LLVM_FOUND AND NOT TARGET ModernLLVM::LLVM)
    ...
    # Component libraries
    if (ModernLLVM_SHARED_LIBS AND TARGET LLVM)
        target_link_libraries(ModernLLVM::LLVM INTERFACE LLVM ${CMAKE_DL_LIBS})
    else ()
        # The components asked for, mapped to their libraries; each imported
        # target carries its own dependencies (Support and the rest), so the
        # transitive closure comes with them. Not LLVM_AVAILABLE_LIBS: that is
        # every library the install has, and with LLVM built as separate
        # shared libraries it includes the ones behind LLVM's own tools --
        # TableGen, FileCheck, the fuzzer driver -- which register command-line
        # options of their own, and two libraries registering `-o` abort every
        # program at start-up with "Option 'o' registered more than once".
